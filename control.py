...
# 150 Yoruba first and last names
sample_first_names = [
...
def generate_parent_data(student_data):
    ...
    for student in student_data:
        parent_income = random.choices([random.randint(70000, 300000), random.randint(80000, 400000)], weights=[90, 10])[0]
...
def generate_library_usage(student_data):
    library_usage = []
    for student in student_data:
        hours_spent = random.uniform(0, 2)
...
def generate_performance_data(student_data):
    ...
    for student in student_data:
        exam_type = 'JAMB'
        score = random.choices([random.randint(130, 149), random.randint(150, 350)], weights=[70, 30])[0]
